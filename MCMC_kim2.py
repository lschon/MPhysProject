# Summary: MCMC_kim2.py
# This is the primary 2 parameter MCMC fit for the Kim et al. data using H_0 and logDeltaM.
# It is only slightly different from MCMC_2var3.py, using different data, but is otherwise the same
# So go to MCMC_2var3.py for detailed notes about what's going on.
...
def run_MCMC2(DeltaM, df_kim, error, minimum=True):
    ...
    def get_datasets():
        ...
        dist = df_kim["R_vc"].to_numpy()
        if minimum:
            V = df_kim["V_min"].to_numpy()
        else:
            V = df_kim["V_max"].to_numpy()
        Verr = np.array(error)
    ...
    def lnprior(theta):
        H_0, logDeltaM = theta
        # Planck constraints for Omega_m
    ...
    def MCMC_plot():
        print("Plotting MCMC plot 1...")
        plt.ion()
        plt.figure(figsize=(20, 20))
        plt.scatter(dist, V, label="Original data", color="black")
        ...
        plt.show()
        print("MCMC plot 1 plotted.")

    MCMC_plot()

    def MCMC_plot2():
        print("Plotting MCMC plot 2...")
        samples = sampler.flatchain
...
def corner_plot(sampler, H_0_true, logDeltaM_true):
    ...
    labels = [r"$H_0$", r"$\log \Delta M$"]
    fig = corner.corner(
        flat_samples, labels=labels, truths=[H_0_true, logDeltaM_true], show_titles=True,
        quantiles=[0.16, 0.5, 0.84])
    fig.set_size_inches((15, 15))
    plt.show()
    print("Corner plot plotted.")
