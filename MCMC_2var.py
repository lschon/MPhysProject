# Summary: MCMC_2var.py
# This conducts the MCMC for logDeltaM and Omega_m for the Millennium Simulation data.
...
def run_MCMC2(DeltaM, df_bins):
    ...
    def lnprior(theta):
        ...
        if 5 < logDeltaM < 25 and 0.001 < Omega_m < 1:
            return 0.0
        else:
            return -np.inf
    ...
    initial = np.array([logDeltaM_true, Omega_m_true])
    ndim = len(initial)
...
def parameter_plot(sampler, ndim):
    ...
    labels = [r"$\log \Delta M$", r"$\Omega_m$"]
    for i in range(ndim):
        ax = axes[i]
...
def present_results(sampler, logDeltaM_true, Omega_m_true, ndim):
    ...
    labelsTeX = [r"$\log \Delta M$", r"$\Omega_m$"]
...
def corner_plot(sampler, logDeltaM_true, Omega_m_true):
    ...
    # labels = ["logDeltaM", "Omega_m"]
    labels = [r"$\log{\Delta M}$", r"$\Omega_{m}$"]
    fig = corner.corner(
